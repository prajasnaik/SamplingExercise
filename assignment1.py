...
def get_x_from_uniform(U: float, a: float, b: float, c: float, cdf_at_c: float) -> float:
    """
...
def generate_triangular(n_samples: int, a: float, b: float, c: float) -> np.ndarray[float]:
    """
...
def generate_pareto_distribution(n_samples: int, xm: float, alpha: int):
    """
...
def get_pareto_value_from_probability(samples: np.ndarray, xm: float, alpha: int) -> np.ndarray[float]:
    """
...
def get_x_from_pareto(U: float, xm: float, alpha: int) -> float:
    """
...
if __name__ == "__main__":
    triangular_samples = generate_triangular(10000, 1, 7, 2)
    pareto_samples = generate_pareto_distribution(10000, 3, 2)
    gamma_samples = generate_gamma_distribution_samples(10000)
